...
(function (global) {
  ...
  function migrate(d) {
    ...
    // TÜM koleksiyonlarda: eskiden yalnız gelir/gider onarılıyordu; hasat ve
    // işçilik kayıtları 1970'e düşüp yıl filtrelerinden/raporlardan kayboluyordu.
    ["giderler", "gelirler", "hasatlar", "isTakip", "isciKiralamalar", "puantaj"].forEach(k => {
      (d[k] || []).forEach(x => {
        if (x && typeof x.tarih === "number" && x.tarih > 20000 && x.tarih < 90000) {
  ...
  // ---- Global aktif yıl (dashboard seçimi + tüm rapor/finans sayfalarına uygulanır) ----
  var _aktifYil;
  function aktifYil() {
    if (_aktifYil === undefined) {
      var raw = null; try { raw = localStorage.getItem("tiys_aktifYil"); } catch (e) {}
      if (raw === null) { var cy = new Date().getFullYear(), ys = yillar(); _aktifYil = (ys.indexOf(cy) >= 0 || !ys.length) ? cy : ys[0]; }
      else _aktifYil = (raw === "" ? null : +raw);
    }
    return _aktifYil; // null = Tüm Yıllar, ya da yıl numarası
  }
  function setAktifYil(y) { _aktifYil = (y == null || y === "") ? null : +y; try { localStorage.setItem("tiys_aktifYil", _aktifYil == null ? "" : String(_aktifYil)); } catch (e) {} }
  function yilUyar(tarih) { var y = aktifYil(); return y == null || yearOf(tarih) === y; }
  ...
  function importJSON(str) {
    try {
      ...
      // Yapısal doğrulama: eskiden HER geçerli JSON ({}, [], alakasız dosya)
      // kabul edilip tüm işletme verisinin üzerine yazılıyordu — tek yanlış
      // dosya seçimi geri dönüşsüz kayıptı.
      if (!o || typeof o !== "object" || Array.isArray(o)) return false;
      if (!Array.isArray(o.tarlalar) && !Array.isArray(o.gelirler) && !Array.isArray(o.giderler)) return false;
      const mevcut = load();
      if (_kayitToplam(o) === 0 && _kayitToplam(mevcut) > 0) return false; // boş içerik dolu veriyi ezemez
      // Tek seferlik geri-dönüş kopyası: içe aktarma yanlışsa son hâl kurtarılabilir
      try { localStorage.setItem(KEY + "_onceki", JSON.stringify(mevcut)); } catch (e) {}
      _data = o; migrate(_data); save(); return true;
  ...
  global.DB = {
    ...
    URUNLER, urun, urunFiyat, setUrunFiyat, sabitFiyat, setSabitFiyat, yilDeger, yillar, aktifYil, setAktifYil, yilUyar, aktifUrunler, urunOptions,

...
(function (global) {
  ...
  function kurulumTs() {
    ...
    let ts = parseInt(t, 10) || Date.now();
    // Saat GELECEĞE kayıtlıysa (yanlış tarih ayarı / saat düzeltmesi) deneme
    // haksız yere "dolmuş" görünüyordu; damgayı şimdiye çek. (Saat İLERİ alınarak
    // yapılan bypass zaten mümkün değil; bu yalnız mağduriyeti önler.)
    if (ts > Date.now()) { ts = Date.now(); set(KUR_KEY, String(ts)); }
    return ts;
  }

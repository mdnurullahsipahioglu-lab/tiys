...
(function (global) {
  ...
  const routes = {
    ...
    "/gelir-hasat": { title: "Hasat Geliri", render: crud({ coll: "gelirler", title: "Hasat Geliri", icon: "🌾", addLabel: "Hasat Geliri Ekle", totalLabel: "Toplam Hasat Geliri", defaults: { tur: "hasat" }, filter: g => g.tur === "hasat" && D.yilUyar(g.tarih), fields: gelirFields, total: r => r.reduce((a, x) => a + (x.tutar || 0), 0), cols: [{ h: "Tarih", v: r => dt(r.tarih) }, { h: "Tarla", v: r => (D.coll("tarlalar").find(t => t.id === r.tarlaId) || {}).ad || "—" }, { h: "Açıklama", v: r => r.aciklama || "—" }, { h: "Tutar", v: r => D.money(r.tutar) }, { h: "USD", v: r => Doviz.usdStr(r.tutar, r.tarih) }] }) },
    "/gelir-kiralama": { title: "İşçi Kiralama Geliri", render: crud({ coll: "gelirler", title: "İşçi Kiralama Geliri", icon: "👷", addLabel: "Tahsilat Ekle", lead: "Müşterilerden alınan işçilik ödemeleri — detaylı borç takibi: İşçi Kiralama sayfası", totalLabel: "Tahsil Edilen", defaults: { tur: "isciKiralama" }, filter: g => g.tur === "isciKiralama" && D.yilUyar(g.tarih), fields: () => [{ key: "tarih", label: "Tarih", type: "date", required: true }, { key: "musteri", label: "Müşteri", type: "text", datalist: D.musteriList(), placeholder: "Ad Soyad" }, { key: "tutar", label: "Alınan Tutar (₺)", type: "money", required: true }, { key: "aciklama", label: "Not", type: "text", full: true, placeholder: "nakit / havale" }], total: r => r.reduce((a, x) => a + (x.tutar || 0), 0), cols: [{ h: "Tarih", v: r => dt(r.tarih) }, { h: "Müşteri", v: r => r.musteri || "—" }, { h: "Açıklama", v: r => r.aciklama || "—" }, { h: "Tutar", v: r => D.money(r.tutar) }, { h: "USD", v: r => Doviz.usdStr(r.tutar, r.tarih) }] }) },
    "/gelir-kira": { title: "Ev Kira Geliri", render: crud({ coll: "gelirler", title: "Ev Kira Geliri", icon: "🏠", addLabel: "Kira Geliri Ekle", totalLabel: "Toplam", defaults: { tur: "evKira" }, filter: g => g.tur === "evKira" && D.yilUyar(g.tarih), fields: gelirFields, total: r => r.reduce((a, x) => a + (x.tutar || 0), 0), cols: [{ h: "Tarih", v: r => dt(r.tarih) }, { h: "Açıklama", v: r => r.aciklama || "—" }, { h: "Tutar", v: r => D.money(r.tutar) }, { h: "USD", v: r => Doviz.usdStr(r.tutar, r.tarih) }] }) },
    ...
    "/gider-tarla": { title: "Tarla Bazlı Giderler", render: giderTarlaOzet },
  ...
  function giderRoute(kategori, ico) {
    ...
    return crud({ coll: "giderler", title: titles[kategori], icon: ico, addLabel: titles[kategori] + " Ekle", totalLabel: "Toplam", defaults: { kategori, altKategori: kategori === "genel" ? "iscilik" : kategori }, filter: g => g.kategori === kategori && D.yilUyar(g.tarih), fields: () => giderFields(kategori), total: r => r.reduce((a, x) => a + (x.tutar || 0), 0), cols: [{ h: "Tarih", v: r => dt(r.tarih) }, { h: "Tür", v: r => r.tur }, { h: "Tarla", v: r => (D.coll("tarlalar").find(t => t.id === r.tarlaId) || {}).ad || "— genel —" }, { h: "Açıklama", v: r => r.aciklama || "—" }, { h: "Tutar", v: r => D.money(r.tutar) }, { h: "USD", v: r => Doviz.usdStr(r.tutar, r.tarih) }] });
  }

  // ---- Tarla bazlı gider özeti (pivot: tarla × kategori) ----
  function giderTarlaOzet(view) {
    const KATS = [["genel", "Genel"], ["makine", "Makine"], ["yakit", "Yakıt"], ["ekipman", "Ekipman"]];
    function render() {
    const _yil = D.aktifYil();   // global yıl (üst çubuktaki seçici) — null = Tüm Yıllar
    const tarlalar = D.coll("tarlalar");
    const giderler = D.coll("giderler").filter(g => D.yilUyar(g.tarih));
    const tMap = {}; tarlalar.forEach(t => tMap[t.id] = t);
    const acc = {};
    const B = id => acc[id] || (acc[id] = { genel: 0, makine: 0, yakit: 0, ekipman: 0, toplam: 0 });
    giderler.forEach(g => {
      const key = !g.tarlaId ? "__genel" : (tMap[g.tarlaId] ? g.tarlaId : "__silinmis");
      const kat = (g.kategori === "makine" || g.kategori === "yakit" || g.kategori === "ekipman") ? g.kategori : "genel";
      const tut = Number(g.tutar) || 0; const b = B(key); b[kat] += tut; b.toplam += tut;
    });
    const bos = { genel: 0, makine: 0, yakit: 0, ekipman: 0, toplam: 0 };
    const tRows = tarlalar.map(t => ({ ad: t.ad, b: acc[t.id] || bos })).sort((a, b) => b.b.toplam - a.b.toplam);
    const extra = [];
    if (acc["__silinmis"]) extra.push({ ad: "— Silinmiş tarla —", b: acc["__silinmis"], muted: true });
    if (acc["__genel"]) extra.push({ ad: "— Tarlasız (genel) —", b: acc["__genel"], muted: true });
    const allRows = tRows.concat(extra);
    const col = { genel: 0, makine: 0, yakit: 0, ekipman: 0, toplam: 0 };
    allRows.forEach(r => { KATS.forEach(([k]) => col[k] += r.b[k]); col.toplam += r.b.toplam; });
    const cell = v => v > 0 ? D.money(v) : `<span style="color:var(--muted,#9ca3af)">—</span>`;
    // Karşılaştırma: gerçek gider + dekar başına + verim (adil kıyas)
    const kars = tarlalar.map(t => {
      const gider = acc[t.id] ? acc[t.id].toplam : 0, alan = Number(t.alanDekar) || 0;
      return { ad: t.ad, alan, gider, gd: alan > 0 ? gider / alan : null, verim: Number(t.verimPuani) || 0 };
    }).sort((a, b) => (b.gd == null ? -1 : b.gd) - (a.gd == null ? -1 : a.gd));
    const gdv = kars.filter(k => k.gd != null && k.gd > 0).map(k => k.gd);
    const gdMax = gdv.length ? Math.max.apply(null, gdv) : null, gdMin = gdv.length ? Math.min.apply(null, gdv) : null;
    const karsRow = k => {
      const hi = k.gd != null && k.gd === gdMax && gdv.length > 1, lo = k.gd != null && k.gd === gdMin && gdv.length > 1;
      const gd = k.gd == null ? `<span style="color:var(--muted,#9ca3af)">—</span>`
        : `<span style="font-weight:700;color:${hi ? "var(--red,#dc2626)" : lo ? "var(--green,#16a34a)" : "inherit"}">${D.money(Math.round(k.gd))}${hi ? " ▲" : lo ? " ▼" : ""}</span>`;
      return `<tr><td style="font-weight:600">${escA(k.ad)}</td>
        <td style="text-align:right">${k.alan ? D.num(k.alan) : "—"}</td>
        <td style="text-align:right">${k.gider > 0 ? D.money(k.gider) : "—"}</td>
        <td style="text-align:right">${gd}</td>
        <td style="text-align:right">${k.verim ? "%" + k.verim : "—"}</td></tr>`;
    };
    view.innerHTML = `
      <div class="page-head"><div><h2 style="margin:0">📊 Tarla Bazlı Giderler</h2>
        <div class="lead">Her tarlanın gideri, kategori kırılımıyla tek tabloda</div></div>
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">${Export.bar('gtarla')}</div></div>
      <div class="kpis" style="grid-template-columns:repeat(3,1fr);max-width:600px">
        ${miniKpi("teal", "Toplam Gider", D.money(col.toplam))}${miniKpi("blue", "Tarla", D.num(tarlalar.length))}${miniKpi("orange", "Gider Kaydı", D.num(giderler.length))}</div>
      <div class="panel" style="margin-top:14px;overflow-x:auto">
        ${tarlalar.length ? `<table class="t"><thead><tr>
            <th>Tarla</th>${KATS.map(([, l]) => `<th style="text-align:right">${l}</th>`).join("")}<th style="text-align:right">TOPLAM</th></tr></thead>
          <tbody>
            ${allRows.map(r => `<tr>
              <td style="${r.muted ? "color:var(--muted,#9ca3af)" : "font-weight:600"}">${escA(r.ad)}</td>
              ${KATS.map(([k]) => `<td style="text-align:right">${cell(r.b[k])}</td>`).join("")}
              <td style="text-align:right;font-weight:700;color:var(--red,#dc2626)">${D.money(r.b.toplam)}</td></tr>`).join("")}
            <tr style="border-top:2px solid var(--line,#e5e7eb)">
              <td style="font-weight:700">TOPLAM</td>
              ${KATS.map(([k]) => `<td style="text-align:right;font-weight:700">${col[k] > 0 ? D.money(col[k]) : "—"}</td>`).join("")}
              <td style="text-align:right;font-weight:800;color:var(--red,#dc2626)">${D.money(col.toplam)}</td></tr>
          </tbody></table>`
        : `<div class="empty"><div class="e-ico">📊</div>Henüz tarla eklenmemiş.<br>Tarla Yönetimi'nden tarla ekle, sonra giderleri tarlaya bağla.</div>`}
      </div>
      ${tarlalar.length ? `<div class="panel" style="margin-top:14px;overflow-x:auto">
        <h3 style="margin:0 0 3px">⚖️ Tarla Karşılaştırma</h3>
        <div class="lead" style="margin-bottom:10px">Dekar başına gider — büyük/küçük fark etmeden tarlaları adil kıyaslar. <span style="color:var(--red,#dc2626)">▲ en pahalı/dekar</span> · <span style="color:var(--green,#16a34a)">▼ en verimli/dekar</span></div>
        <table class="t"><thead><tr><th>Tarla</th><th style="text-align:right">Alan (dekar)</th><th style="text-align:right">Toplam Gider</th><th style="text-align:right">Gider/dekar</th><th style="text-align:right">Verim %</th></tr></thead>
          <tbody>${kars.map(karsRow).join("")}</tbody></table></div>` : ""}`;
    Export.wire(view, 'gtarla', () => ({
      file: "TIYS-Tarla-Giderler-" + (_yil || "TumYillar"), title: "TİYS — Tarla Bazlı Giderler (" + (_yil || "Tüm Yıllar") + ")",
      tables: [{
        name: "Tarla Giderleri", headers: ["Tarla", "Genel", "Makine", "Yakıt", "Ekipman", "Toplam"],
        rows: allRows.map(r => [r.ad, Math.round(r.b.genel), Math.round(r.b.makine), Math.round(r.b.yakit), Math.round(r.b.ekipman), Math.round(r.b.toplam)])
          .concat([["TOPLAM", Math.round(col.genel), Math.round(col.makine), Math.round(col.yakit), Math.round(col.ekipman), Math.round(col.toplam)]])
      }, {
        name: "Tarla Karşılaştırma", headers: ["Tarla", "Alan (dekar)", "Toplam Gider", "Gider/dekar", "Verim %"],
        rows: kars.map(k => [k.ad, k.alan || 0, Math.round(k.gider), k.gd != null ? Math.round(k.gd) : "", k.verim || ""])
      }]
    }));
    }
    render();
  ...
  function bugunISO2() { try { const d = new Date(); return d.getFullYear() + "-" + ("0" + (d.getMonth() + 1)).slice(-2) + "-" + ("0" + d.getDate()).slice(-2); } catch (e) { return "2026-06-17"; } } // yerel gün: UTC gece 00-03 kaymasi fixi
  ...
  function tarlaDetay(tarla, refreshList) {
    const ov = document.createElement("div"); ov.className = "modal-overlay";
    function build() {
      const gid = D.coll("giderler").filter(g => g.tarlaId === tarla.id && D.yilUyar(g.tarih));
      const gel = D.coll("gelirler").filter(g => g.tarlaId === tarla.id && D.yilUyar(g.tarih));
  ...
  function ayarlarPage(view) {
    ...
    view.querySelector("#b_xls_file").onchange = e => {
      ...
      Export.importExcel(file, (err, sheets) => {
        ...
        const d = D.load();
        // Normalize: Excel'den gelen değerler ham geliyordu — seri-numarası
        // tarihler (42797 → 1970'e düşüp raporlardan kayboluyordu) ve Türkçe
        // "1.250,50" tutarlar (Number() → NaN → 0 TL) sessizce bozuluyordu.
        const normTarih = (v) => {
          if (typeof v === "number" && v > 20000 && v < 90000) return new Date(Math.round((v - 25569) * 86400000)).toISOString().slice(0, 10);
        ...
        const SAYI_ALAN = ["tutar", "miktar", "fiyat", "adet", "gun", "yevmiye", "toplam", "kg", "alan", "donum"];
        apply.forEach(a => {
          a[2].forEach(o => {
            if (!o.id) o.id = "imp_" + Math.random().toString(36).slice(2, 9);
            if (o.tarih != null) o.tarih = normTarih(o.tarih);
            SAYI_ALAN.forEach(f => { if (o[f] != null && o[f] !== "") o[f] = normSayi(o[f]); });
          });
          d[a[0]] = a[2];
        });
    ...
    view.querySelector("#b_reset").onclick = () => { if (confirm("Girilen GÜNLÜK kayıtlar (gelir, gider, hasat, iş, puantaj, işçi kiralama) silinecek.\n\nÇeşitler/türler, sabit fiyatlar, tarlalar ve ayarlar KORUNACAK." + (window.Cloud && Cloud.status().paired ? "\n\n⚠ Eşleştirme açık: bu silme BULUT yedeğine ve bağlı diğer cihaza da yayılır." : "") + "\n\nEmin misin?")) { D.gunlukTemizle(); Forms.toast("Günlük kayıtlar temizlendi (çeşitler ve tarlalar korundu)"); location.hash = "#/dashboard"; FIYS.route(); } };
    var bFull = view.querySelector("#b_full"); if (bFull) bFull.onclick = () => { if (confirm("DİKKAT: Çeşitler/türler, sabit fiyatlar, tarlalar ve ayarlar DAHİL girilen HER ŞEY kalıcı silinecek; program sıfırdan, boş başlayacak (yeni işletme / satış)." + (window.Cloud && Cloud.status().paired ? "\n\n⚠ Eşleştirme açık: sıfırlama BULUT yedeğini ve bağlı diğer cihazı da etkiler." : "") + "\n\nEmin misin?")) { D.bosla(); Forms.toast("Her şey sıfırlandı — yeni işletme"); location.hash = "#/dashboard"; FIYS.route(); } };
  ...
  // ---- Global yıl seçici (üst çubuk) — dashboard + tüm rapor/finans sayfalarına uygulanır ----
  function syncYil() {
    var sel = document.getElementById("yilGlobal");
    if (!sel) return;
    var cur = D.aktifYil();
    sel.innerHTML = '<option value="">🗓️ Tüm Yıllar</option>' + D.yillar().map(function (y) { return '<option value="' + y + '">🗓️ ' + y + '</option>'; }).join("");
    sel.value = cur == null ? "" : String(cur);
    sel.onchange = function () { D.setAktifYil(sel.value ? +sel.value : null); route(); };
  }
  ...
  function paywallGoster(view) {
    view.innerHTML = `
      <div class="panel" style="max-width:520px;margin:36px auto;text-align:center;padding:30px 24px">
        ...
        <button class="btn" id="lis_yedek" style="width:100%;padding:10px;margin-top:10px">💾 Verilerimi yedek olarak indir</button>
    ...
    // Veri hapsini önle: deneme dolduğunda kullanıcının KENDİ girdiği veriye
    // erişimi tamamen kesiliyordu (dışa aktarma da kilitliydi). Kilit ekranından
    // her zaman JSON yedek alınabilir.
    view.querySelector("#lis_yedek").onclick = () => {
      try {
        const blob = new Blob([D.exportJSON()], { type: "application/json" });
        const url = URL.createObjectURL(blob);
        const aEl = document.createElement("a");
        aEl.href = url; aEl.download = "tiys-yedek.json"; aEl.click();
        setTimeout(() => URL.revokeObjectURL(url), 3000);
        Forms.toast("Yedek indirildi ✓");
      } catch (e) { alert("Yedek alınamadı: " + e); }
    };
    const dene = () => {
  ...
  global.FIYS = { toggleSidebar, closeSidebar, route, cikisYap, senkronGoster, syncYil };

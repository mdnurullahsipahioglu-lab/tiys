...
(function (global) {
  ...
  // Bu ikisi KALICI tutulur: yalnız bellekte kalınca, çevrimdışı girilen
  // kayıtlar uygulama kapanıp açıldığında unutuluyor ve açılıştaki otomatik
  // pull eski bulut verisini üzerine yazıyordu (tarla girişleri kayboluyordu).
  const BEKLEYEN_KEY = "tiys_bekleyen_push";
  const SON_BULUT_KEY = "tiys_son_bulut";
  function bekleyenYaz(v) { try { if (v) localStorage.setItem(BEKLEYEN_KEY, "1"); else localStorage.removeItem(BEKLEYEN_KEY); } catch (e) {} }
  function sonBulutYaz(ts) { try { if (ts) localStorage.setItem(SON_BULUT_KEY, ts); } catch (e) {} }
  ...
  async function restore() {
    ...
    try { yerelDegisti = localStorage.getItem(BEKLEYEN_KEY) === "1"; } catch (e) {}
    try { sonBilinenBulut = localStorage.getItem(SON_BULUT_KEY) || null; } catch (e) {}
    if (kod) {
      setDurum("bagli");
      // Gönderilmemiş yerel değişiklik varsa önce onu buluta it; pull etseydik
      // bulut eski verisi çevrimdışı girilen kayıtları ezerdi.
      if (yerelDegisti) { await push(); } else { await pull(true); }
      baglantiKur();
    }
  ...
  function kopar() {
    ...
    bekleyenYaz(false);
    try { localStorage.removeItem(SON_BULUT_KEY); } catch (e) {}
  ...
  async function push() {
    ...
    // Boş yerel, DOLU bulutu ezemez (pull'daki korumanın simetriği): sıfırlama
    // veya bozulmuş yerel veri, tek push'la ortak bulut yedeğini siliyordu.
    if (payload.sayi === 0) {
      try {
        const mevcutRow = await pantryOku(basketAdi(kod));
        if (mevcutRow && (mevcutRow.sayi || 0) > 0) {
          yerelDegisti = false; bekleyenYaz(false); setDurum("bagli");
          return { ok: true, atlandi: true };
        }
      } catch (e) { /* okunamadıysa normal akış */ }
    }
    try {
      await pantryYaz(basketAdi(kod), payload);
      sonBilinenBulut = payload.guncelleme; sonBulutYaz(payload.guncelleme);
      yerelDegisti = false; bekleyenYaz(false);
      lastSync = new Date(); setDurum("bagli"); return { ok: true };
  ...
  global.addEventListener("tiys:save", function () {
    ...
    yerelDegisti = true; bekleyenYaz(true);
